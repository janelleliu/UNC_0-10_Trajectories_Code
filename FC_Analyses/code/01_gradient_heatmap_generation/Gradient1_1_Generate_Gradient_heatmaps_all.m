...
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Gradient1_Gradients_results_AlignedToHCP/'];  %%!!    %%!!  NEED CHANGE!  %%!!    %%!!  
if ~exist(outputpath, 'dir')
    mkdir(outputpath);
end
listpath0 = '/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/lists_0.3mm/';  %%!!  
listpath1 = '/media/zhark2/glab6/Project_Replication/Preprocessed_Data/UNC/lists_0.3mm/';  %%!!  
...
for aa = 1:num_a
    ...
    subjects = importSubjIDs([listpath0 age_s '_full_subject_updated_final.txt']);  %%!!  
    ...
    for ss = 1:num_s
        subj = subjects{ss};
        outfile = [outputdir subj '_' measure '_Heatmap_' age '.1D'];  %%!!  vector %% got from python scripts  
        outfile1 = [outputdir subj '_' measure '_Heatmap_' age '.nii.gz'];  %%!!  map
        if ~exist(outfile1, 'file') && exist(outfile, 'file')
            MeasureM = load(outfile);
            W = zeros(size(W_voxelpar), 'single');
            for vv=1:length(MeasureM)
                W(W_voxelpar==vv) = MeasureM(vv);
            end
            nii.img = W; %% 
    ...
    %}
    if strcmp(age, 'HCP')
        outfile_mean = [outputpath measure '_Mean_Heatmap_' age '.1D'];  %%!!  vector
        outfile_mean1 = [outputpath measure '_Mean_Heatmap_' age '.nii.gz'];  %%!!  map
    else
        outfile_mean = [outputpath measure '_Mean_twinPick_32W_Healthy_Heatmap_' age '.1D'];  %%!!  vector  %%!!  NEED CHANGE!
        outfile_mean1 = [outputpath measure '_Mean_twinPick_32W_Healthy_Heatmap_' age '.nii.gz'];  %%!!  map  %%!!  NEED CHANGE!
    end
    if ~exist(outfile_mean1, 'file')
        %
        if strcmp(age_s, 'HCP')
            subjects = importSubjIDs([listpath0 age_s '_full_subject_updated_final.txt']);  %%!!  HCP
        else
            subjects = importSubjIDs([listpath1 age_s '_full_subject_updated_final_twinPick_32W_Healthy.txt']);  %%!!    %%!!  Normal  
        end
        num_s = length(subjects);
        MeasureM_mean = zeros(num_voxelpar, 1);
        for ss = 1:num_s
            subj = subjects{ss};
            outfile = [outputdir subj '_' measure '_Heatmap_' age '.1D'];  %%!!  vector
            MeasureM = load(outfile);
            MeasureM_mean = MeasureM_mean + MeasureM;
        end
        MeasureM_mean = MeasureM_mean / num_s; %% 
        dlmwrite(outfile_mean, MeasureM_mean, 'delimiter', '\t');  %%!!  
        %}
