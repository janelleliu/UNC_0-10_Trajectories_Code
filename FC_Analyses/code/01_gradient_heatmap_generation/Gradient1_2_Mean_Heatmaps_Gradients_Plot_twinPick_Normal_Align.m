...
datapath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Gradient1_Gradients_results_AlignedToHCP/'];  %%!!  Original MeanHeatmaps
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Gradient1_Gradients_results_AlignedToHCP/gradientsplots_results/'];  %%!!  %%!!  
...
for gg = 1:length(groups)
    ...
    if strcmp(group, 'HCP')
        infile1 = [datadir 'Gradient1_AlignedToHCP_Mean_Heatmap_' group '.nii.gz'];  %%!!  
        infile2 = [datadir 'Gradient2_AlignedToHCP_Mean_Heatmap_' group '.nii.gz'];  %%!!  
    else
        infile1 = [datadir 'Gradient1_AlignedToHCP_Mean_twinPick_32W_Healthy_Heatmap_' group '.nii.gz'];  %%!!  
        infile2 = [datadir 'Gradient2_AlignedToHCP_Mean_twinPick_32W_Healthy_Heatmap_' group '.nii.gz'];  %%!!  mean of the individual gradient maps
    end
    ...
    hold off
    if strcmp(group, 'HCP')
        outfile1 = [outputpath measure label '_Mean_Heatmap_Plot_' group '.fig'];  %%!!  
    else
        outfile1 = [outputpath measure label '_twinPick_32W_Healthy_Mean_Heatmap_Plot_' group '.fig'];  %%!!  
    end
    saveas(gcf, outfile1);  %%!!  
    if strcmp(group, 'HCP')
        outfile2 = [outputpath measure label '_Mean_Heatmap_Plot_' group '.png'];  %%!!  
    else
        outfile2 = [outputpath measure label '_twinPick_32W_Healthy_Mean_Heatmap_Plot_' group '.png'];  %%!!  mean of the individual gradient maps
    end
    saveas(gcf, outfile2);  %%!!  
    close; %% 
