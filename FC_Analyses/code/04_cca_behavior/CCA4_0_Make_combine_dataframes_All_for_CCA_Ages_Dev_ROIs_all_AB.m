...
for mm = 1:length(measures)
    ...
    for gg = 1:length(groups)
        ...
        subjects_twinPick_AllBehavior = intersect(intersect(subjects_twinPick_IQ, subjects_twinPick_Anxiety), subjects_twinPick_Depression);  %%!!  %%!!  subjects with all 3
        Medu_twinPick_AllBehavior = Medu_all(ismember(subjects_all, subjects_twinPick_AllBehavior));  %%!!    %%!!  If need IN ORDER by using ISMEMBER, need to SORT every subject list first!!!!!!!!        
        subjects_twinPick_AllBehavior_withMedu = subjects_twinPick_AllBehavior(~isnan(Medu_twinPick_AllBehavior));  %%!!  with AllBehavior & with Medu  %%!!  
        Medu_twinPick_AllBehavior_withMedu = Medu_all(ismember(subjects_all, subjects_twinPick_AllBehavior_withMedu));  %%!!    %%!!  
        Medu_twinPick_AllBehavior_withMedu = Medu_twinPick_AllBehavior_withMedu - mean(Medu_twinPick_AllBehavior_withMedu);  %%!!    %%!!  centered
        NDD_twinPick_AllBehavior_withMedu = double(ismember(subjects_twinPick_AllBehavior_withMedu, subjects_twinPick_NDD));  %%!!    %%!!  categorized
        MPD_twinPick_AllBehavior_withMedu = double(ismember(subjects_twinPick_AllBehavior_withMedu, subjects_twinPick_MPD));  %%!!    %%!!  categorized
        GAAssess_twinPick_AllBehavior_withMedu = cell2mat(GAAssess_all(ismember(subjects_all, subjects_twinPick_AllBehavior_withMedu)));  %%!!    %%!!  
        GAAssess_twinPick_AllBehavior_withMedu = GAAssess_twinPick_AllBehavior_withMedu - mean(GAAssess_twinPick_AllBehavior_withMedu);  %%!!    %%!!  centered
        C_twinPick_AllBehavior_withMedu = CC(ismember(subjects_twinPick_All, subjects_twinPick_AllBehavior_withMedu), :);  %%!!    %%!!  categorized/centered GAB+GAS+BW+Sex for subjects_twinPick_AllBehavior_withMedu  %%!!  %%!!  
        C_twinPick_AllBehavior_withMedu = [C_twinPick_AllBehavior_withMedu, Medu_twinPick_AllBehavior_withMedu, NDD_twinPick_AllBehavior_withMedu, MPD_twinPick_AllBehavior_withMedu, GAAssess_twinPick_AllBehavior_withMedu];  %%!!    %%!!  All covs for longitudinal regression test (except for intercept 1+)
        IQ_twinPick_AllBehavior_withMedu = IQ_all(ismember(subjects_all, subjects_twinPick_AllBehavior_withMedu));  %%!!    %%!!  no need to center
        Anxiety_twinPick_AllBehavior_withMedu = Anxiety_all(ismember(subjects_all, subjects_twinPick_AllBehavior_withMedu));  %%!!    %%!!  no need to center
        Depression_twinPick_AllBehavior_withMedu = Depression_all(ismember(subjects_all, subjects_twinPick_AllBehavior_withMedu));  %%!!    %%!!  no need to center
        measures_feature_Dev_twinPick_AllBehavior_withMedu = measures_feature_Dev(ismember(subjects_twinPick_All, subjects_twinPick_AllBehavior_withMedu), :);  %%!!  %%!!  Modify
        X = measures_feature_Dev_twinPick_AllBehavior_withMedu;  %%!!  %%!!  Modify
        Y = [IQ_twinPick_AllBehavior_withMedu, Anxiety_twinPick_AllBehavior_withMedu, Depression_twinPick_AllBehavior_withMedu];  %%!!  %%!!  
        C = C_twinPick_AllBehavior_withMedu;  %%!!  
        outdir = [outputpath 'AllBehavior' measure_YR '_' level '_' measure label Cov_label '_' group '/data/'];  %%!!  %%!!  
        if ~exist(outdir, 'dir')
            mkdir(outdir);
        end
        save([outdir 'X.mat'], 'X');
        save([outdir 'Y.mat'], 'Y');
        save([outdir 'C.mat'], 'C');
        Normal_I = ismember(subjects_twinPick_AllBehavior_withMedu, subjects_twinPick_Normal);  %%!!  
...
L = cell(1 + size(Y, 2), 4);
L(1, :) = {'Label', 'Category', 'Label_proc', 'Label_bold'};
labelsY = {['IQ' measure_YR], ['Anxiety' measure_YR], ['Depression' measure_YR]};  %%!!  %%!!  
groupsY = {['Cognition_' measure_YR 'YR'], ['Emotion_' measure_YR 'YR'], ['Emotion_' measure_YR 'YR']};  %%!!  %%!!  
for yy = 1:size(Y, 2)
    labelY = labelsY{yy};  %%!!  
    groupY = groupsY{yy};  %%!!  
    L{1 + yy, 1} = labelY; %% 
...
T = array2table(L); %% 
writetable(T, [outputpath 'LabelsY_AllBehavior' measure_YR '.csv'], 'WriteVariableNames', false);  %%!!  %%!!  
