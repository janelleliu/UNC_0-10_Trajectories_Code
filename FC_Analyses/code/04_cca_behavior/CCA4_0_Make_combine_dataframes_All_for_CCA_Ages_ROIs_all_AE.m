...
level = 'ROIs_all';  %%!!  %%!!  2yr funPar 278 ROIs, all sub-measures
...
for mm = 1:length(measures)
    ...
    for gg = 1:length(groups)
        ...
        subjects_twinPick_AllEmotion10 = intersect(subjects_twinPick_Anxiety10, subjects_twinPick_Depression10);  %%!!  %%!!  subjects with all 2 Emotions
        Medu_twinPick_AllEmotion10 = Medu_all(ismember(subjects_all, subjects_twinPick_AllEmotion10));  %%!!    %%!!  If need IN ORDER by using ISMEMBER, need to SORT every subject list first!!!!!!!!        
        subjects_twinPick_AllEmotion10_withMedu = subjects_twinPick_AllEmotion10(~isnan(Medu_twinPick_AllEmotion10));  %%!!  with AllEmotion10 & with Medu  %%!!  
        Medu_twinPick_AllEmotion10_withMedu = Medu_all(ismember(subjects_all, subjects_twinPick_AllEmotion10_withMedu));  %%!!    %%!!  
        Medu_twinPick_AllEmotion10_withMedu = Medu_twinPick_AllEmotion10_withMedu - mean(Medu_twinPick_AllEmotion10_withMedu);  %%!!    %%!!  centered
        NDD_twinPick_AllEmotion10_withMedu = double(ismember(subjects_twinPick_AllEmotion10_withMedu, subjects_twinPick_NDD));  %%!!    %%!!  categorized
        MPD_twinPick_AllEmotion10_withMedu = double(ismember(subjects_twinPick_AllEmotion10_withMedu, subjects_twinPick_MPD));  %%!!    %%!!  categorized
        GAAssess10_twinPick_AllEmotion10_withMedu = cell2mat(GAAssess10_all(ismember(subjects_all, subjects_twinPick_AllEmotion10_withMedu)));  %%!!    %%!!  
        GAAssess10_twinPick_AllEmotion10_withMedu = GAAssess10_twinPick_AllEmotion10_withMedu - mean(GAAssess10_twinPick_AllEmotion10_withMedu);  %%!!    %%!!  centered
        C_twinPick_AllEmotion10_withMedu = CC(ismember(subjects_twinPick_All, subjects_twinPick_AllEmotion10_withMedu), :);  %%!!    %%!!  categorized/centered GAB+GAS+BW+Sex for subjects_twinPick_AllEmotion10_withMedu  %%!!  %%!!  
        C_twinPick_AllEmotion10_withMedu = [C_twinPick_AllEmotion10_withMedu, Medu_twinPick_AllEmotion10_withMedu, NDD_twinPick_AllEmotion10_withMedu, MPD_twinPick_AllEmotion10_withMedu, GAAssess10_twinPick_AllEmotion10_withMedu];  %%!!    %%!!  All covs for longitudinal regression test (except for intercept 1+)
        %IQ10_twinPick_AllEmotion10_withMedu = IQ10_all(ismember(subjects_all, subjects_twinPick_AllEmotion10_withMedu));  %%!!    %%!!  no need to center
        Anxiety10_twinPick_AllEmotion10_withMedu = Anxiety10_all(ismember(subjects_all, subjects_twinPick_AllEmotion10_withMedu));  %%!!    %%!!  no need to center
        Depression10_twinPick_AllEmotion10_withMedu = Depression10_all(ismember(subjects_all, subjects_twinPick_AllEmotion10_withMedu));  %%!!    %%!!  no need to center
        measures_feature_twinPick_AllEmotion10_withMedu = measures_feature(ismember(subjects_twinPick_All, subjects_twinPick_AllEmotion10_withMedu), :);  %%!!  
        X = measures_feature_twinPick_AllEmotion10_withMedu;  %%!!  
        Y = [Anxiety10_twinPick_AllEmotion10_withMedu, Depression10_twinPick_AllEmotion10_withMedu];  %%!!  %%!!  
        C = C_twinPick_AllEmotion10_withMedu;  %%!!  
        outdir = [outputpath 'AllEmotion10_' level '_' measure label Cov_label '_' group '/data/'];  %%!!  %%!!  
        if ~exist(outdir, 'dir')
            mkdir(outdir);
        end
        save([outdir 'X.mat'], 'X');
...
L = cell(1 + size(Y, 2), 4);
L(1, :) = {'Label', 'Category', 'Label_proc', 'Label_bold'};
labelsY = {'Anxiety10', 'Depression10'};  %%!!  %%!!  'IQ10', 
groupsY = {'Emotion_10YR', 'Emotion_10YR'};  %%!!  %%!!  'Cognition_10YR', 
for yy = 1:size(Y, 2)
    labelY = labelsY{yy};  %%!!  
    groupY = groupsY{yy};  %%!!  
    L{1 + yy, 1} = labelY; %% 
...
T = array2table(L); %% 
writetable(T, [outputpath 'LabelsY_AllEmotion10.csv'], 'WriteVariableNames', false);  %%!!  %%!!  
