...
for mm = 1:length(measures)
    ...
    for gg = 1:length(groups)
        ...
        subjects_twinPick_AllCognition = intersect(intersect(subjects_twinPick_IQ, subjects_twinPick_WM_BRIEF), subjects_twinPick_WM_SB);  %%!!  %%!!  subjects with all 3
        Medu_twinPick_AllCognition = Medu_all(ismember(subjects_all, subjects_twinPick_AllCognition));  %%!!    %%!!  If need IN ORDER by using ISMEMBER, need to SORT every subject list first!!!!!!!!        
        subjects_twinPick_AllCognition_withMedu = subjects_twinPick_AllCognition(~isnan(Medu_twinPick_AllCognition));  %%!!  with AllBehavior & with Medu  %%!!  
        Medu_twinPick_AllCognition_withMedu = Medu_all(ismember(subjects_all, subjects_twinPick_AllCognition_withMedu));  %%!!    %%!!  
        Medu_twinPick_AllCognition_withMedu = Medu_twinPick_AllCognition_withMedu - mean(Medu_twinPick_AllCognition_withMedu);  %%!!    %%!!  centered
        NDD_twinPick_AllCognition_withMedu = double(ismember(subjects_twinPick_AllCognition_withMedu, subjects_twinPick_NDD));  %%!!    %%!!  categorized
        MPD_twinPick_AllCognition_withMedu = double(ismember(subjects_twinPick_AllCognition_withMedu, subjects_twinPick_MPD));  %%!!    %%!!  categorized
        GAAssess_twinPick_AllCognition_withMedu = cell2mat(GAAssess_all(ismember(subjects_all, subjects_twinPick_AllCognition_withMedu)));  %%!!    %%!!  
        GAAssess_twinPick_AllCognition_withMedu = GAAssess_twinPick_AllCognition_withMedu - mean(GAAssess_twinPick_AllCognition_withMedu);  %%!!    %%!!  centered
        C_twinPick_AllCognition_withMedu = CC(ismember(subjects_twinPick_All, subjects_twinPick_AllCognition_withMedu), :);  %%!!    %%!!  categorized/centered GAB+GAS+BW+Sex for subjects_twinPick_AllBehavior_withMedu  %%!!  %%!!  
        C_twinPick_AllCognition_withMedu = [C_twinPick_AllCognition_withMedu, Medu_twinPick_AllCognition_withMedu, NDD_twinPick_AllCognition_withMedu, MPD_twinPick_AllCognition_withMedu, GAAssess_twinPick_AllCognition_withMedu];  %%!!    %%!!  All covs for longitudinal regression test (except for intercept 1+)
        IQ_twinPick_AllCognition_withMedu = IQ_all(ismember(subjects_all, subjects_twinPick_AllCognition_withMedu));  %%!!    %%!!  no need to center
        WM_BRIEF_twinPick_AllCognition_withMedu = WM_BRIEF_all(ismember(subjects_all, subjects_twinPick_AllCognition_withMedu));  %%!!    %%!!  no need to center
        WM_SB_twinPick_AllCognition_withMedu = WM_SB_all(ismember(subjects_all, subjects_twinPick_AllCognition_withMedu));  %%!!    %%!!  no need to center
        measures_feature_Dev_twinPick_AllCognition_withMedu = measures_feature_Dev(ismember(subjects_twinPick_All, subjects_twinPick_AllCognition_withMedu), :);  %%!!  %%!!  Modify
        X = measures_feature_Dev_twinPick_AllCognition_withMedu;  %%!!  %%!!  Modify
        Y = [IQ_twinPick_AllCognition_withMedu, WM_BRIEF_twinPick_AllCognition_withMedu, WM_SB_twinPick_AllCognition_withMedu];  %%!!  %%!!  
        C = C_twinPick_AllCognition_withMedu;  %%!!  
        outdir = [outputpath 'AllCognition1' measure_YR '_' level '_' measure label Cov_label '_' group '/data/'];  %%!!  %%!!  
        if ~exist(outdir, 'dir')
            mkdir(outdir);
        end
        save([outdir 'X.mat'], 'X');
        save([outdir 'Y.mat'], 'Y');
        save([outdir 'C.mat'], 'C');
        Normal_I = ismember(subjects_twinPick_AllCognition_withMedu, subjects_twinPick_Normal);  %%!!  
...
L = cell(1 + size(Y, 2), 4);
L(1, :) = {'Label', 'Category', 'Label_proc', 'Label_bold'};
labelsY = {['IQ' measure_YR], ['WM_BRIEF' measure_YR], ['WM_SB' measure_YR]};  %%!!  %%!!  
groupsY = {['Cognition_' measure_YR 'YR'], ['Cognition_' measure_YR 'YR'], ['Cognition_' measure_YR 'YR']};  %%!!  %%!!  
for yy = 1:size(Y, 2)
    labelY = labelsY{yy};  %%!!  
    groupY = groupsY{yy};  %%!!  
    L{1 + yy, 1} = labelY; %% 
...
T = array2table(L); %% 
writetable(T, [outputpath 'LabelsY_AllCognition1' measure_YR '.csv'], 'WriteVariableNames', false);  %%!!  %%!!  
