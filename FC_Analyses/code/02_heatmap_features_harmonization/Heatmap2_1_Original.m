...
if ismember(measure, {'Gradient1_AlignedToHCP', 'Gradient2_AlignedToHCP'})
...
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Heatmap2_New_heatmaps_results/' measure label '/'];   %%!!   %%!!  
...
for s=1:num_subj
    subj = subjects{s};
    % measure  
    infile = [datadir subj '_' measure '_Heatmap_' group '.nii.gz'];  %%!!    %%!!  
    outfile = [outdir subj '_' measure '_Heatmap_' group '.nii.gz'];  %%!!    %%!!  
    if ~exist(outfile, 'file')
...
if strcmp(group, 'HCP')
    outfile_mean1 = [outputpath measure label '_Mean_Heatmap_' group '.nii.gz'];  %%!!  map
else
    outfile_mean1 = [outputpath measure label '_Mean_twinPick_32W_Healthy_Heatmap_' group '.nii.gz'];  %%!!  map
end
