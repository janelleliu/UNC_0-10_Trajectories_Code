...
measures = {'GradientsDispN2'};  %%!!    %%!!  run all subjects together: McorrZ / McosSim2 / Str(_8mm2) / Enodal(_8mm2) / Elocal(_8mm2) / TGradient1 / Gradient1_AlignedToFouryear / Gradient2_AlignedToFouryear 
...
for ss = 1:num_subj  %%!!  
    ...
    ms = mean(gradients2_wb, 1);  %%!!    %%!!  
    gradients2_wb = gradients2_wb - ms;  %%!!    %%!!  
    ns = norm(gradients2_wb, 'fro');  %%!!    %%!!  
    gradients2_wb = gradients2_wb / ns;  %%!!    %%!!  Normalized by Mean and Matrix Frobenius Norm    
    for rr = 1:num_ROIs
        ROI = ROIs{rr};
        network = networks{rr};  %%!!  only used for visualization  
        M = gradients2_wb(inds_8net==rr, :);  %%!!    %%!!  
        centroid = mean(M, 1);
