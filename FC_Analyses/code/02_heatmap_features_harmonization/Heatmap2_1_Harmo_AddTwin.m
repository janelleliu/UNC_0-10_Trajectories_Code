...
groups = {'neonate', 'oneyear', 'twoyear', 'fouryear', 'sixyear', 'eightyear', 'tenyear'};  %%!!  separately  , 'HCP'
groups_s = {'0', '1', '2', '4', '6', '8', '10'};  %%!!  separately  , 'HCP'
...
load(['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/M_FC_Z_Covs_All/M_FC_Z_Covs_All_0.3mm_notr90_UNC_' group '.mat']);  %%!!  C as Covariates  _twinPick  _twinPick
C_P = C(:, [2, 4]);  %%!!  Covs to Preserve: GAS and Sex
C_R = C(:, 9:10);  %%!!  Covs to Remove: DeCov for meanFD and scan-length (centered) here
datapath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Heatmap2_New_heatmaps_results_AddTwin/0_ComBat/' measure label '/'];   %%!!    
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Heatmap2_New_heatmaps_results_AddTwin/' measure label '_Harmo/'];   %%!!   %%!!  
...
if strcmp(group, 'HCP')
    ...
    subjects = importSubjIDs([listpath group_s '_full_subject_updated_final.txt']);  %%!!    twinPick All    _twinPick
end
