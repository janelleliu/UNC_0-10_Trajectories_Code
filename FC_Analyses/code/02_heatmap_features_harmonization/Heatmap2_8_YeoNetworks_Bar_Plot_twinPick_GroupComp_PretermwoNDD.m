...
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/Heatmap2_New_heatmaps_results/barplots_results_GroupComp_Preterm_woNDD/' measure label Cov_label '/'];  %%!!    %%!!  
...
for gg = 1:length(groups)
    ...
    subjects_twinPick_Normal_32W = importSubjIDs(['/media/zhark2/glab6/Project_Replication/Preprocessed_Data/UNC/lists_0.3mm/' group_s '_full_subject_updated_final_twinPick_32W_Healthy.txt']);  %%!!    %%!!  Normal_32W  
    subjects_twinPick_Preterm_32W = importSubjIDs(['/media/zhark2/glab6/Project_Replication/Preprocessed_Data/UNC/lists_0.3mm/' group_s '_full_subject_updated_final_twinPick_Preterm_32W_woNDD.txt']);  %%!!    %%!!  Preterm_32W  
    ...
    inds_Normal_32W = ismember(subjects_twinPick_All, subjects_twinPick_Normal_32W);  %%!!  
    inds_Preterm_32W = ismember(subjects_twinPick_All, subjects_twinPick_Preterm_32W);  %%!!  
    ...
    load(infile);
    measures_network_Normal_32W = measures_network(inds_Normal_32W, :);  %%!!  
    measures_network_Preterm_32W = measures_network(inds_Preterm_32W, :);  %%!!  
    %measures_network_MPD = measures_network(inds_MPD, :);  %%!!  
    
    measures_network_Normal_32W_mean = mean(measures_network_Normal_32W, 1);
    measures_network_Normal_32W_sem = std(measures_network_Normal_32W, 1) / sqrt(size(measures_network_Normal_32W, 1));
    measures_network_Preterm_32W_mean = mean(measures_network_Preterm_32W, 1);
    measures_network_Preterm_32W_sem = std(measures_network_Preterm_32W, 1) / sqrt(size(measures_network_Preterm_32W, 1));
    ...
    [~, rank_inds] = sort(measures_network_Normal_32W_mean, 'descend');  %%!!  ranked by Normal_32W mean
    networks_ranked = networks(rank_inds);
    measures_network_Normal_32W_mean_ranked = measures_network_Normal_32W_mean(rank_inds);
    measures_network_Normal_32W_sem_ranked = measures_network_Normal_32W_sem(rank_inds);
    measures_network_Preterm_32W_mean_ranked = measures_network_Preterm_32W_mean(rank_inds);
    measures_network_Preterm_32W_sem_ranked = measures_network_Preterm_32W_sem(rank_inds);
    ...
    x = 1:num_ROIs;  %%!!  
    y = [measures_network_Normal_32W_mean_ranked; measures_network_Preterm_32W_mean_ranked];  %%!!  Normal-Preterm 32W
    err = [measures_network_Normal_32W_sem_ranked; measures_network_Preterm_32W_sem_ranked];  %%!!  Normal-Preterm 32W
    figure;  %%!!  
    hold on
    ...
    ylabel(['Mean ' measure_str ' Normal-Preterm 32W']);  %%!!    %%!!  
    %title([group]); %% 
    set(gca, 'fontsize', 15, 'FontWeight', 'bold');  %%!!  Updated
    hold off
    outfile1 = [outputpath measure label '_Mean_twinPick_Normal_Preterm_32W_Heatmap_Ranked_Bar_' group Cov_label '.fig'];  %%!!    %%!!  Normal-Preterm 32W    
    saveas(gcf, outfile1);  %%!!  
    outfile2 = [outputpath measure label '_Mean_twinPick_Normal_Preterm_32W_Heatmap_Ranked_Bar_' group Cov_label '.png'];  %%!!    %%!!  Normal-Preterm 32W    
    saveas(gcf, outfile2);  %%!!  
    close; %% 
    %{
    x = 1:num_ROIs;  %%!!  
    y = [measures_network_Normal_32W_mean_ranked; measures_network_MPD_mean_ranked];  %%!!  Normal_32W-MPD
    err = [measures_network_Normal_32W_sem_ranked; measures_network_MPD_sem_ranked];  %%!!  Normal_32W-MPD
    figure;  %%!!  
    hold on
    ...
    ylim(ylims(mm,:)); %%   %%!!  %%!!  %%!!  
    %xlabel('Ranked Network');  %%!!  %%!!  
    ylabel(['Mean ' measure_str ' Normal_32W-MPD']);  %%!!    %%!!  
    %title([group]); %% 
    set(gca, 'fontsize', 13);  %%!!  
    hold off
    outfile1 = [outputpath measure label '_Mean_twinPick_Normal_32W_MPD_Heatmap_Ranked_Bar_' group Cov_label '.fig'];  %%!!    %%!!  Normal_32W-MPD    
    saveas(gcf, outfile1);  %%!!  
    outfile2 = [outputpath measure label '_Mean_twinPick_Normal_32W_MPD_Heatmap_Ranked_Bar_' group Cov_label '.png'];  %%!!    %%!!  Normal_32W-MPD    
    saveas(gcf, outfile2);  %%!!  
    close; %% 
    %}
end
