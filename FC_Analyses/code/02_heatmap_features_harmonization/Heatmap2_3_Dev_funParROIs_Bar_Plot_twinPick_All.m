...
level = 'Dev_ROIs';  %%!!  %%!!  2yr funPar 278 ROIs
...
for gg = 1:length(groups)
    ...
    measures_ROI_norm = zeros(num_subj, num_ROIs);  %%!!  %%!!  
    measures_ROI_std_norm = zeros(num_subj, num_ROIs);  %%!!  %%!!  
    normfile = [normpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_M_Original_0.3mm_notr90_UNC.mat'];
    load(normfile); %% X_GAS_Day; Individual_Curves_Mean_twinPick_Normal; Individual_Curves_Std_twinPick_Normal
    for ss = 1:num_subj
        subj = subjects{ss};
        x_GAS = C_raw(ss, 2);  %%!!  %%!!  
        for rr = 1:num_ROIs
            measure_ROI_norm = interp1(X_GAS_Day, Individual_Curves_Mean_twinPick_Normal(:, rr), x_GAS, 'spline');  %%!!  %%!!  
            measure_ROI_std_norm = interp1(X_GAS_Day, Individual_Curves_Std_twinPick_Normal(:, rr), x_GAS, 'spline');  %%!!  %%!!  
            measures_ROI_norm(ss, rr) = measure_ROI_norm;  %%!!  %%!!  
            measures_ROI_std_norm(ss, rr) = measure_ROI_std_norm;  %%!!  %%!!  
        end
    end
    measures_ROI_Dev = (measures_ROI - measures_ROI_norm) ./ measures_ROI_std_norm;  %%!!  %%!!  
    measures_ROI = measures_ROI_Dev;  %%!!  %%!!  
