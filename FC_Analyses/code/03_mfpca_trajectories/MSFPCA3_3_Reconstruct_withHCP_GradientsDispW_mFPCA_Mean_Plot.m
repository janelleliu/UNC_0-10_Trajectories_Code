...
networks = {'Whole-brain', 'Whole-brain'};  %% 1    %%!!  updated!!  just use one, two is for mFPCA    
num_ROIs = length(ROIs); % 

Colors = [255 0 0; 255 0 0] / 255;  %%!!  RGB values for Whole-brain  

measures = {'GradientsDispW', 'GradientsDispW2'};  %%!!    %%!!    %%!!  AlignedToFouryear_NoScale
measures_str = {'Dispersion', 'Dispersion-Normalized'};  %%!!    %%!!  
Cov_label = '_Harmo';  %%!!    %%!!  ''(original) / '_Harmo'  %%!!    %%!!  
if strcmp(Cov_label, '')
    ylims = [1,2.5; 0.006,0.009];  %%!!    %%!!  
...
for rr = 1:1%num_ROIs  %%!!    %%!!  only show one    
...
legend(networks(1), 'Location', 'northeastoutside', 'fontsize', 20, 'FontWeight', 'bold');  %%!!  Updated  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean Trajectory ' measure_str ' Normal']);  %%!!  label Cov_label  twinPick  %%!!  Updated
set(gcf, 'position', [500, 500, 1200, 800]);  %%!!    %%!!  [500, 500, 1000, 800]
set(gca, 'fontsize', 20, 'FontWeight', 'bold');  %%!!  Updated
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
...
legend([strcat('Normal-', networks(1)), strcat('NDD-', networks(1))], 'Location', 'northeastoutside', 'fontsize', 18, 'FontWeight', 'bold');  %%!!  Updated
std_dev = Individual_Curves_Std_twinPick_NDD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_NDD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
legend([strcat('Normal-', networks(1)), strcat('MPD-', networks(1))], 'Location', 'northeastoutside', 'fontsize', 18, 'FontWeight', 'bold');  %%!!  Updated
std_dev = Individual_Curves_Std_twinPick_MPD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_MPD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
