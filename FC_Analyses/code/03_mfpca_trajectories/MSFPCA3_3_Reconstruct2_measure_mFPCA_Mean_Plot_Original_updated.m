...
if strcmp(Cov_label, '')
    ylims = [0,200; 0.15,0.35; 0.2,0.45; 0.15,0.55; -3,3; -3,3];  %%!!    %%!!  ; 1.5,3; 0.005,0.008
...
for mm=5:6%1:length(measures)  %%!!    %%!!  7:8%
    measure = measures{mm};
...
subjects_twinPick_AD = importSubjIDs([listpath '01246810_Union_full_subject_updated_final_twinPick_AD.txt']);  %%!!  %%!!  
subjects_twinPick_AU = importSubjIDs([listpath '01246810_Union_full_subject_updated_final_twinPick_AU.txt']);  %%!!  %%!!  
...
FPC_scores_twinPick_AD = cell2mat(FPC_scores(ismember(FPC_scores(:, 2), subjects_twinPick_AD), 4:end));  %%!!  %%!!  
FPC_scores_twinPick_AU = cell2mat(FPC_scores(ismember(FPC_scores(:, 2), subjects_twinPick_AU), 4:end));  %%!!  %%!!  
...
Individual_Curves_twinPick_AD = zeros(length(subjects_twinPick_AD), n_T, num_ROIs);
for ii = 1:length(subjects_twinPick_AD)
    for pp = 1:num_ROIs
        Individual_Curve = Y_STDZ_Mean_mFPCA(:, pp);
        for kk = 1:n_FPC
            Individual_Curve = Individual_Curve + Y_STDZ_FPCs_mFPCA(:, (pp-1)*n_FPC+kk)*FPC_scores_twinPick_AD(ii, (pp-1)*n_FPC+kk);
        end
        Individual_Curve = Individual_Curve * Var_std(pp) + Var_mean(pp);  %%!!  Re-scale
        Individual_Curves_twinPick_AD(ii, :, pp) = Individual_Curve;  %%!!  
    end
end
Individual_Curves_Mean_twinPick_AD = squeeze(mean(Individual_Curves_twinPick_AD, 1));
Individual_Curves_Std_twinPick_AD = squeeze(std(Individual_Curves_twinPick_AD, 0, 1));  %%!!  

Individual_Curves_twinPick_AU = zeros(length(subjects_twinPick_AU), n_T, num_ROIs);
for ii = 1:length(subjects_twinPick_AU)
    for pp = 1:num_ROIs
        Individual_Curve = Y_STDZ_Mean_mFPCA(:, pp);
        for kk = 1:n_FPC
            Individual_Curve = Individual_Curve + Y_STDZ_FPCs_mFPCA(:, (pp-1)*n_FPC+kk)*FPC_scores_twinPick_AU(ii, (pp-1)*n_FPC+kk);
        end
        Individual_Curve = Individual_Curve * Var_std(pp) + Var_mean(pp);  %%!!  Re-scale
        Individual_Curves_twinPick_AU(ii, :, pp) = Individual_Curve;  %%!!  
    end
end
Individual_Curves_Mean_twinPick_AU = squeeze(mean(Individual_Curves_twinPick_AU, 1));
Individual_Curves_Std_twinPick_AU = squeeze(std(Individual_Curves_twinPick_AU, 0, 1));  %%!!  
...
X_inds = X_GAS_Day>=300 & X_GAS_Day<=3950;  %%!!  
x = X_GAS_Day(X_inds); %% 
y = Individual_Curves_Mean_twinPick_AD(X_inds, :);  %%!!    %%!!  
for rr = 1:num_ROIs
    plot(x, y(:, rr), 'Color', Colors(rr, :), 'LineStyle', '--', 'LineWidth', linewidth);  %%    %%!!  'Marker', 'o', 
end
%legend(strcat('Normal-', networks), 'Location', 'northeastoutside'); %% 
std_dev = Individual_Curves_Std_twinPick_AD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_AD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
for rr = 1:num_ROIs
    fill([x; flipud(x)], [upper_bound(:, rr); flipud(lower_bound(:, rr))], Colors(rr, :), 'FaceAlpha', facealpha, 'EdgeColor', 'none', 'HandleVisibility', 'off');  %%!!    %%!!  
end
y = Individual_Curves_Mean_twinPick_AU(X_inds, :);  %%!!    %%!!  
for rr = 1:num_ROIs
    plot(x, y(:, rr), 'Color', Colors(rr, :), 'LineStyle', ':', 'LineWidth', linewidth);  %%    %%!!  'Marker', 'o', 
end
legend([strcat('ADHD-', networks), strcat('Autism-', networks)], 'Location', 'northeastoutside'); %%   %%!!  
std_dev = Individual_Curves_Std_twinPick_AU(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_AU));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
for rr = 1:num_ROIs
    fill([x; flipud(x)], [upper_bound(:, rr); flipud(lower_bound(:, rr))], Colors(rr, :), 'FaceAlpha', facealpha, 'EdgeColor', 'none', 'HandleVisibility', 'off');  %%!!    %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean mFPCA ' measure_str ' twinPick ADHD-Autism']);  %%!!  label Cov_label 
set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
set(gca, 'fontsize', 20);  %%!!  
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
...
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_NDD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_ADHD_Autism_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
...
legend([strcat('Normal-', networks), strcat('ADHD-', networks), strcat('Autism-', networks)], 'Location', 'northeastoutside'); %%   %%!!  
std_dev = Individual_Curves_Std_twinPick_AU(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_AU));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean mFPCA ' measure_str ' twinPick Normal-ADHD-Autism']);  %%!!  label Cov_label 
set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
set(gca, 'fontsize', 20);  %%!!  
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
...
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_NDD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_ADHD_Autism_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
