...
legend(networks(1), 'Location', 'northeastoutside'); %%   %%!!    %%!!  only show one    
...
legend([strcat('Normal-', networks(1)), strcat('NDD-', networks(1))], 'Location', 'northeastoutside'); %%   %%!!    %%!!    %%!!  only show one    
std_dev = Individual_Curves_Std_twinPick_NDD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_NDD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
legend([strcat('Normal-', networks(1)), strcat('MPD-', networks(1))], 'Location', 'northeastoutside'); %%   %%!!    %%!!    %%!!  only show one    
std_dev = Individual_Curves_Std_twinPick_MPD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_MPD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
