...
outputpath = '/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/MSFPCA3_New_tables_results/mFPCA/mfpca_plot_results_Reconstruct_withHCP_37W/';  %%!!  %%!!  
...
subjects_twinPick_Normal_37W = importSubjIDs([listpath '01246810_Union_full_subject_updated_final_twinPick_37W_Healthy.txt']);  %%!!  37W    
...
FPC_scores_twinPick_Normal_37W = cell2mat(FPC_scores(ismember(FPC_scores(:, 2), subjects_twinPick_Normal_37W), 4:end));  %%!!  
...
Individual_Curves_twinPick_Normal_37W = zeros(length(subjects_twinPick_Normal_37W), n_T, num_ROIs);
for ii = 1:length(subjects_twinPick_Normal_37W)
    for pp = 1:num_ROIs
        Individual_Curve = Y_STDZ_Mean_mFPCA(:, pp);
        for kk = 1:n_FPC
            Individual_Curve = Individual_Curve + Y_STDZ_FPCs_mFPCA(:, (pp-1)*n_FPC+kk)*FPC_scores_twinPick_Normal_37W(ii, (pp-1)*n_FPC+kk);
        end
        Individual_Curve = Individual_Curve * Var_std(pp) + Var_mean(pp);  %%!!  Re-scale
        Individual_Curves_twinPick_Normal_37W(ii, :, pp) = Individual_Curve;  %%!!  
    end
end
Individual_Curves_Mean_twinPick_Normal_37W = squeeze(mean(Individual_Curves_twinPick_Normal_37W, 1));
Individual_Curves_Std_twinPick_Normal_37W = squeeze(std(Individual_Curves_twinPick_Normal_37W, 0, 1));  %%!!  
...
X_inds = X_GAS_Day>=300 & X_GAS_Day<=3950;  %%!!  
x = X_GAS_Day(X_inds); %% 
y = Individual_Curves_Mean_twinPick_Normal_37W(X_inds, :);  %%!!    %%!!  
for rr = 1:num_ROIs
    plot(x, y(:, rr), 'Color', Colors(rr, :), 'LineStyle', '-', 'LineWidth', linewidth);  %%    %%!!  'Marker', 'o', 
end
legend(networks, 'Location', 'northeastoutside'); %% 
std_dev = Individual_Curves_Std_twinPick_Normal_37W(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_Normal_37W));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
for rr = 1:num_ROIs
    fill([x; flipud(x)], [upper_bound(:, rr); flipud(lower_bound(:, rr))], Colors(rr, :), 'FaceAlpha', facealpha, 'EdgeColor', 'none', 'HandleVisibility', 'off');  %%!!    %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean mFPCA ' measure_str ' twinPick Normal-37W']);  %%!!  label Cov_label 
set(gcf, 'position', [500, 500, 1200, 800]);  %%!!    %%!!  [500, 500, 1000, 800]
set(gca, 'fontsize', 20);  %%!!  
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
end
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_37W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_37W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
...
legend([strcat('Normal-37W-', networks), strcat('NDD-', networks)], 'Location', 'northeastoutside'); %%   %%!!  
std_dev = Individual_Curves_Std_twinPick_NDD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_NDD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean mFPCA ' measure_str ' twinPick Normal-37W-NDD']);  %%!!  label Cov_label 
set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
set(gca, 'fontsize', 20);  %%!!  
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
end
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_37W_NDD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_37W_NDD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
...
legend([strcat('Normal-37W-', networks), strcat('MPD-', networks)], 'Location', 'northeastoutside'); %%   %%!!  
std_dev = Individual_Curves_Std_twinPick_MPD(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_MPD));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean mFPCA ' measure_str ' twinPick Normal-37W-MPD']);  %%!!  label Cov_label 
set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
set(gca, 'fontsize', 20);  %%!!  
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
end
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_37W_MPD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_37W_MPD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
