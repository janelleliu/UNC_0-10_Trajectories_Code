...
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/MSFPCA3_New_tables_results/mFPCA/mfpca_results_GroupComp/' measure label Cov_label '/'];  %%!!    %%!!  
...
for gg = 1:length(groups)
    ...
    subjects_twinPick_AD = importSubjIDs([listpath group_s '_full_subject_updated_final_twinPick_AD.txt']);  %%!!    %%!!  AD  
    subjects_twinPick_AU = importSubjIDs([listpath group_s '_full_subject_updated_final_twinPick_AU.txt']);  %%!!    %%!!  AU  
    ...
    AD_twinPick_UNNM_withMedu = double(ismember(subjects_twinPick_UNNM_withMedu, subjects_twinPick_AD));  %%!!    %%!!  categorized
    AU_twinPick_UNNM_withMedu = double(ismember(subjects_twinPick_UNNM_withMedu, subjects_twinPick_AU));  %%!!    %%!!  categorized
    ...
    C_twinPick_UNNM_withMedu = [C_twinPick_UNNM_withMedu, Medu_twinPick_UNNM_withMedu, AD_twinPick_UNNM_withMedu, AU_twinPick_UNNM_withMedu, MPD_twinPick_UNNM_withMedu];  %%!!    %%!!  All covs for longitudinal regression test (except for intercept 1+)
    ...
    FPC_1_scores_twinPick_UNNM_withMedu = FPC_scores_twinPick_UNNM_withMedu(:, 1:n_FPC:end);  %%!!    %%!!  1/2/3/4  1/2/3/4    
    %{
    ...
    %}
    Bs_AD = zeros(1, num_ROIs);
    Ps_AD = zeros(1, num_ROIs);
    Bs_AU = zeros(1, num_ROIs);
    Ps_AU = zeros(1, num_ROIs);
    Bs_MPD = zeros(1, num_ROIs);
    Ps_MPD = zeros(1, num_ROIs);
    alpha = 0.05;  %%!!  
    for rr = 1:num_ROIs
        y_twinPick_UNNM_withMedu = FPC_1_scores_twinPick_UNNM_withMedu(:, rr);  %%!!  1/2/3/4/Dev    
        ...
        Bs_AD(rr) = mdl.Coefficients.Estimate(6);  %%!!    %%!!  AD
        Ps_AD(rr) = mdl.Coefficients.pValue(6);  %%!!    %%!!  AD
        Bs_AU(rr) = mdl.Coefficients.Estimate(7);  %%!!    %%!!  AU
        Ps_AU(rr) = mdl.Coefficients.pValue(7);  %%!!    %%!!  AU
        Bs_MPD(rr) = mdl.Coefficients.Estimate(8);  %%!!    %%!!  MPD
        Ps_MPD(rr) = mdl.Coefficients.pValue(8);  %%!!    %%!!  MPD
    end
    Hs_AD = Ps_AD <= alpha;
    Ps_AD_FDR = mafdr(Ps_AD, 'BHFDR', true);
    Hs_AD_FDR = Ps_AD_FDR <= alpha;
    Hs_AU = Ps_AU <= alpha;
    Ps_AU_FDR = mafdr(Ps_AU, 'BHFDR', true);
    Hs_AU_FDR = Ps_AU_FDR <= alpha;
    Hs_MPD = Ps_MPD <= alpha;
    Ps_MPD_FDR = mafdr(Ps_MPD, 'BHFDR', true);
    Hs_MPD_FDR = Ps_MPD_FDR <= alpha;
    outfile = [outputpath measure label '_mFPCA_twinPick_Normal_AD_AU_MPD_FPC_1_scores_RegressionP_' group Cov_label '.mat'];  %%!!    %%!!  Normal-AD/-AU/-MPD    1/2/3/4/Dev    
    save(outfile, 'ROIs', 'networks', 'subjects_twinPick_UNNM_withMedu', 'FPC_scores_twinPick_UNNM_withMedu', 'FPC_1_scores_twinPick_UNNM_withMedu', 'C_twinPick_UNNM_withMedu', 'Bs_AD', 'Ps_AD', 'Hs_AD', 'Ps_AD_FDR', 'Hs_AD_FDR', 'Bs_AU', 'Ps_AU', 'Hs_AU', 'Ps_AU_FDR', 'Hs_AU_FDR', 'Bs_MPD', 'Ps_MPD', 'Hs_MPD', 'Ps_MPD_FDR', 'Hs_MPD_FDR');  %%!!    %%!!  1/2/3/4/Dev    
    
    for rr = 1:num_ROIs
        ROI = ROIs{rr};
        network = networks{rr};
        y_twinPick_UNNM_withMedu = FPC_1_scores_twinPick_UNNM_withMedu(:, rr);  %%!!  1/2/3/4/Dev    
        y_twinPick_UNNM_withMedu_Normal = y_twinPick_UNNM_withMedu(ismember(subjects_twinPick_UNNM_withMedu, subjects_twinPick_Normal));
        y_twinPick_UNNM_withMedu_AD = y_twinPick_UNNM_withMedu(ismember(subjects_twinPick_UNNM_withMedu, subjects_twinPick_AD));
        y_twinPick_UNNM_withMedu_AU = y_twinPick_UNNM_withMedu(ismember(subjects_twinPick_UNNM_withMedu, subjects_twinPick_AU));
        y_twinPick_UNNM_withMedu_MPD = y_twinPick_UNNM_withMedu(ismember(subjects_twinPick_UNNM_withMedu, subjects_twinPick_MPD));
        figure;
        Violin({y_twinPick_UNNM_withMedu_Normal}, 1, 'ShowMean', true);
        Violin({y_twinPick_UNNM_withMedu_AD}, 2, 'ShowMean', true);
        Violin({y_twinPick_UNNM_withMedu_AU}, 3, 'ShowMean', true);
        grouporder = {'Normal', 'ADHD', 'Autism'};
        xticks([1, 2, 3]); %% 
        ...
        saveas(gcf, [outputpath measure label '_mFPCA_twinPick_Normal_AD_AU_FPC_1_scores_ViolinPlot_' group Cov_label '_' network '.png']);  %%!!  1/2/3/4/Dev    
        close;    
%         figure;
%         Violin({y_twinPick_UNNM_withMedu_Normal}, 1, 'ShowMean', true);
%         Violin({y_twinPick_UNNM_withMedu_MPD}, 2, 'ShowMean', true);
...
%         close;  
    ...
    Ps_MPD_FDR = mafdr(Ps_MPD, 'BHFDR', true);
    Hs_MPD_FDR = Ps_MPD_FDR <= alpha;
    outfile = [outputpath measure label '_mFPCA_twinPick_Normal_AD_AU_MPD_FPC_2_scores_RegressionP_' group Cov_label '.mat'];  %%!!    %%!!  Normal-NDD/-MPD    1/2/3/4/Dev    
    save(outfile, 'ROIs', 'networks', 'subjects_twinPick_UNNM_withMedu', 'FPC_scores_twinPick_UNNM_withMedu', 'FPC_2_scores_twinPick_UNNM_withMedu', 'C_twinPick_UNNM_withMedu', 'Bs_AD', 'Ps_AD', 'Hs_AD', 'Ps_AD_FDR', 'Hs_AD_FDR', 'Bs_AU', 'Ps_AU', 'Hs_AU', 'Ps_AU_FDR', 'Hs_AU_FDR', 'Bs_MPD', 'Ps_MPD', 'Hs_MPD', 'Ps_MPD_FDR', 'Hs_MPD_FDR');  %%!!    %%!!  1/2/3/4/Dev    
    ...
    for rr = 1:num_ROIs
        ...
        set(gcf,'Units','pixels','Position',[200 200 560 420]);
        set(gca, 'FontSize', 12, 'FontWeight', 'bold');  %%!!    %%!!  
        saveas(gcf, [outputpath measure label '_mFPCA_twinPick_Normal_AD_AU_FPC_2_scores_ViolinPlot_' group Cov_label '_' network '.png']);  %%!!  1/2/3/4/Dev    
        close;    
%         figure;
%         Violin({y_twinPick_UNNM_withMedu_Normal}, 1, 'ShowMean', true);
%         Violin({y_twinPick_UNNM_withMedu_MPD}, 2, 'ShowMean', true);
