...
outputpath = ['/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/MSFPCA3_New_tables_results/mFPCA/mfpca_results_GroupComp_noBW/' measure label Cov_label '/'];  %%!!    %%!!  
...
for gg = 1:length(groups)
    ...
    C_twinPick_UNNM_withMedu = C(ismember(subjects_twinPick_All, subjects_twinPick_UNNM_withMedu), :);  %%!!    %%!!  categorized/centered GAB+Sex for subjects_twinPick_UNNM_withMedu
    ...
    for rr = 1:num_ROIs
        ...
        Bs_NDD(rr) = mdl.Coefficients.Estimate(5);  %%!!    %%!!  NDD
        Ps_NDD(rr) = mdl.Coefficients.pValue(5);  %%!!    %%!!  NDD
        Bs_MPD(rr) = mdl.Coefficients.Estimate(6);  %%!!    %%!!  MPD
        Ps_MPD(rr) = mdl.Coefficients.pValue(6);  %%!!    %%!!  MPD
    end
    Hs_NDD = Ps_NDD <= alpha;
    ...
    outfile = [outputpath measure label '_mFPCA_twinPick_Normal_NDD_MPD_FPC_1_scores_RegressionP_' group Cov_label '.mat'];  %%!!    %%!!  Normal-NDD/-MPD    1/2/3/4/Dev    
    ...
    for rr = 1:num_ROIs
        ...
        saveas(gcf, [outputpath measure label '_mFPCA_twinPick_Normal_NDD_FPC_1_scores_ViolinPlot_' group Cov_label '_' network '.png']);  %%!!  1/2/3/4/Dev    
        close;    
        figure;
        Violin({y_twinPick_UNNM_withMedu_Normal}, 1, 'ShowMean', true);
    ...
    for rr = 1:num_ROIs
        ...
        set(gcf,'Units','pixels','Position',[200 200 560 420]);
        set(gca, 'FontSize', 12, 'FontWeight', 'bold');  %%!!    %%!!  
        saveas(gcf, [outputpath measure label '_mFPCA_twinPick_Normal_NDD_FPC_2_scores_ViolinPlot_' group Cov_label '_' network '.png']);  %%!!  1/2/3/4/Dev    
        close;    
        figure;
        Violin({y_twinPick_UNNM_withMedu_Normal}, 1, 'ShowMean', true);
        Violin({y_twinPick_UNNM_withMedu_MPD}, 2, 'ShowMean', true);
