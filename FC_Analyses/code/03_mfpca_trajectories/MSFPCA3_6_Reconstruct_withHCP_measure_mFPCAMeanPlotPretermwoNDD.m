...
measures_str = {'Str', 'Enodal', 'Elocal', 'McosSim', 'Gradient1', 'Gradient2', 'Dispersion', 'Dispersion-Normal_32Wized'};  %%!!    %%!!  
...
outputpath = '/media/zhark2/glab7/Haitao/UNC_Trajectory_heatmaps/0NewCompleteAnalyses/MSFPCA3_New_tables_results/mFPCA/mfpca_plot_results_Reconstruct_withHCP_Preterm_woNDD/';  %%!!  %%!!  
...
subjects_twinPick_Normal_32W = importSubjIDs([listpath '01246810_Union_full_subject_updated_final_twinPick_32W_Healthy.txt']);
subjects_twinPick_Preterm_32W = importSubjIDs([listpath '01246810_Union_full_subject_updated_final_twinPick_Preterm_32W_woNDD.txt']);
...
FPC_scores_twinPick_Normal_32W = cell2mat(FPC_scores(ismember(FPC_scores(:, 2), subjects_twinPick_Normal_32W), 4:end));  %%!!  
FPC_scores_twinPick_Preterm_32W = cell2mat(FPC_scores(ismember(FPC_scores(:, 2), subjects_twinPick_Preterm_32W), 4:end));  %%!!  
...
Individual_Curves_twinPick_Normal_32W = zeros(length(subjects_twinPick_Normal_32W), n_T, num_ROIs);
for ii = 1:length(subjects_twinPick_Normal_32W)
    for pp = 1:num_ROIs
        Individual_Curve = Y_STDZ_Mean_mFPCA(:, pp);
        for kk = 1:n_FPC
            Individual_Curve = Individual_Curve + Y_STDZ_FPCs_mFPCA(:, (pp-1)*n_FPC+kk)*FPC_scores_twinPick_Normal_32W(ii, (pp-1)*n_FPC+kk);
        end
        Individual_Curve = Individual_Curve * Var_std(pp) + Var_mean(pp);  %%!!  Re-scale
        Individual_Curves_twinPick_Normal_32W(ii, :, pp) = Individual_Curve;  %%!!  
    end
end
Individual_Curves_Mean_twinPick_Normal_32W = squeeze(mean(Individual_Curves_twinPick_Normal_32W, 1));
Individual_Curves_Std_twinPick_Normal_32W = squeeze(std(Individual_Curves_twinPick_Normal_32W, 0, 1));  %%!!  

Individual_Curves_twinPick_Preterm_32W = zeros(length(subjects_twinPick_Preterm_32W), n_T, num_ROIs);
for ii = 1:length(subjects_twinPick_Preterm_32W)
    for pp = 1:num_ROIs
        Individual_Curve = Y_STDZ_Mean_mFPCA(:, pp);
        for kk = 1:n_FPC
            Individual_Curve = Individual_Curve + Y_STDZ_FPCs_mFPCA(:, (pp-1)*n_FPC+kk)*FPC_scores_twinPick_Preterm_32W(ii, (pp-1)*n_FPC+kk);
        end
        Individual_Curve = Individual_Curve * Var_std(pp) + Var_mean(pp);  %%!!  Re-scale
        Individual_Curves_twinPick_Preterm_32W(ii, :, pp) = Individual_Curve;  %%!!  
    end
end
Individual_Curves_Mean_twinPick_Preterm_32W = squeeze(mean(Individual_Curves_twinPick_Preterm_32W, 1));
Individual_Curves_Std_twinPick_Preterm_32W = squeeze(std(Individual_Curves_twinPick_Preterm_32W, 0, 1));  %%!!  
...
X_inds = X_GAS_Day>=300 & X_GAS_Day<=3950;  %%!!  
x = X_GAS_Day(X_inds); %% 
y = Individual_Curves_Mean_twinPick_Normal_32W(X_inds, :);  %%!!    %%!!  
for rr = 1:num_ROIs
    plot(x, y(:, rr), 'Color', Colors(rr, :), 'LineStyle', '-', 'LineWidth', linewidth);  %%    %%!!  'Marker', 'o', 
end
legend(networks, 'Location', 'northeastoutside', 'fontsize', 20, 'FontWeight', 'bold');  %%!!  Updated
std_dev = Individual_Curves_Std_twinPick_Normal_32W(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_Normal_32W));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
for rr = 1:num_ROIs
    fill([x; flipud(x)], [upper_bound(:, rr); flipud(lower_bound(:, rr))], Colors(rr, :), 'FaceAlpha', facealpha, 'EdgeColor', 'none', 'HandleVisibility', 'off');  %%!!    %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean Trajectory ' measure_str ' Normal']);  %%!!  label Cov_label  twinPick 32W  %%!!  Updated
set(gcf, 'position', [500, 500, 1200, 800]);  %%!!    %%!!  [500, 500, 1000, 800]
set(gca, 'fontsize', 20, 'FontWeight', 'bold');  %%!!  Updated
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
end
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_32W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_32W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
...
X_inds = X_GAS_Day>=300 & X_GAS_Day<=3950;  %%!!  
x = X_GAS_Day(X_inds); %% 
y = Individual_Curves_Mean_twinPick_Preterm_32W(X_inds, :);  %%!!    %%!!  
for rr = 1:num_ROIs
    plot(x, y(:, rr), 'Color', Colors(rr, :), 'LineStyle', '-', 'LineWidth', linewidth);  %%    %%!!  'Marker', 'o', 
end
legend(networks, 'Location', 'northeastoutside', 'fontsize', 20, 'FontWeight', 'bold');  %%!!  Updated
std_dev = Individual_Curves_Std_twinPick_Preterm_32W(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_Preterm_32W));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
for rr = 1:num_ROIs
    fill([x; flipud(x)], [upper_bound(:, rr); flipud(lower_bound(:, rr))], Colors(rr, :), 'FaceAlpha', facealpha, 'EdgeColor', 'none', 'HandleVisibility', 'off');  %%!!    %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean Trajectory ' measure_str ' Preterm']);  %%!!  label Cov_label  twinPick 32W  %%!!  Updated
set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
set(gca, 'fontsize', 20, 'FontWeight', 'bold');  %%!!  Updated
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
end
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Preterm_32W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Preterm_32W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
...
%legend(strcat('Normal_32W-', networks), 'Location', 'northeastoutside'); %% 
std_dev = Individual_Curves_Std_twinPick_Normal_32W(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_Normal_32W));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
legend([strcat('Normal 32W-', networks), strcat('Preterm 32W-', networks)], 'Location', 'northeastoutside', 'fontsize', 18, 'FontWeight', 'bold');  %%!!  Updated
std_dev = Individual_Curves_Std_twinPick_Preterm_32W(X_inds, :);  %%!!    %%!!  
sem = std_dev / sqrt(length(subjects_twinPick_Preterm_32W));  %%!!  standard error mean
upper_bound = y + sem;  %%!!  
lower_bound = y - sem;  %%!!  
...
%yticks([-0.2:0.1:1.0]);  %%!!  
xlabel('GAS Day');
ylabel(['Mean Trajectory ' measure_str ' Normal-Preterm']);  %%!!  label Cov_label  twinPick 32W  %%!!  Updated
set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
set(gca, 'fontsize', 18, 'FontWeight', 'bold');  %%!!  Updated
if strcmp(measure, 'Gradient2_AlignedToHCP')
    set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
end
%outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_Preterm_32W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
%saveas(gcf, outfile);  %%!!  
outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_Preterm_32W_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
saveas(gcf, outfile0);  %%!!  
clf(gcf);
close(gcf);
...
%     plot(x, y(:, rr), 'Color', Colors(rr, :), 'LineStyle', '--', 'LineWidth', linewidth);  %%    %%!!  'Marker', 'o', 
% end
% legend([strcat('Normal_32W-', networks), strcat('MPD-', networks)], 'Location', 'northeastoutside'); %%   %%!!  
% std_dev = Individual_Curves_Std_twinPick_MPD(X_inds, :);  %%!!    %%!!  
% sem = std_dev / sqrt(length(subjects_twinPick_MPD));  %%!!  standard error mean
% upper_bound = y + sem;  %%!!  
% lower_bound = y - sem;  %%!!  
...
% %yticks([-0.2:0.1:1.0]);  %%!!  
% xlabel('GAS Day');
% ylabel(['Mean mFPCA ' measure_str ' twinPick Normal_32W-MPD']);  %%!!  label Cov_label 
% set(gcf, 'position', [500, 500, 1000, 800]);  %%!!    %%!!  [500, 500, 1200, 800]
% set(gca, 'fontsize', 20);  %%!!  
% if strcmp(measure, 'Gradient2_AlignedToHCP')
%     set(gca, 'YDir', 'reverse');  %%!!  reverse Transmodal-to-Primary to Primary-to-Transmodal
% end
% %outfile = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_32W_MPD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.fig']; %%   %%!!  
% %saveas(gcf, outfile);  %%!!  
% outfile0 = [outputpath 'Mean_mFPCA_' measure label Cov_label '_twinPick_Normal_32W_MPD_Plot_updated_Original' FD '_notr' num2str(threshold) '_' dataset '.png']; %%   %%!!  
% saveas(gcf, outfile0);  %%!!  
% clf(gcf);
% close(gcf);
